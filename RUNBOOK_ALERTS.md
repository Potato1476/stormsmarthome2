...
## Chạy thử tự động & trực quan (Khuyên dùng)

Bạn có thể chạy file script tương tác tiện lợi vừa được tạo ở thư mục gốc để thực hiện toàn bộ quá trình test:
```bash
./run_alerts_test.sh
```
Script này cung cấp menu chọn để bạn:
1. Test trực tiếp webhook (bằng Maven + Java).
2. Khởi động Docker Gateway & MQTT Broker.
3. Bơm dữ liệu giả lập vượt ngưỡng 2500W.
4. Bơm dữ liệu thực tế từ bất kỳ file CSV nào trong folder `iot-data-publisher`.
5. Theo dõi logs và dọn dẹp các container.

---

## Chạy thử thủ công bằng lệnh từng bước (1 gateway, dữ liệu giả lập)
...
### Kiểm tra nhanh thủ công chỉ riêng webhook (không cần Storm)
